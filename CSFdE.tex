    ...
    \begin{tikzpicture}
        \node [mybox] (box){%
            \begin{minipage}{0.3\textwidth}
                ...
                \footnotesize
                \textbf{\color{blue}Tempo di Propagazione - Algoritmo Universale}
                ...
                $\bullet$ Scarica ($V_{out}\!\downarrow$): \textbf{nMOS}

                $\bullet$ Carica ($V_{out}\!\uparrow$): \textbf{pMOS}

                \vspace{0.1cm}
                \textbf{2. Calcola $V_{pinch}$}
                $$V_{pinch} = V_{GS} - V_T$$
                {\tiny (pMOS: $|V_{GS}| - |V_T|$)}
                \end{minipage}
                ...
                \textbf{3. DECISION TREE} -- Confronta $[V_{start}, V_{end}]$ con $V_{pinch}$:

                \vspace{0.1cm}
                \scriptsize
                \textbf{\underline{CASO A: Tutto in SAT}} ($V_{out} > V_{pinch}$ sempre)

                Modello: generatore $I_{sat}$ costante
                $\quad \boxed{\Delta t = \frac{C_L \cdot \Delta V}{I_{sat}}}$ con $I_{sat} = K(V_{GS}-V_T)^2$

                \vspace{0.1cm}
                \textbf{\underline{CASO B: Tutto in OHMICA}} ($V_{out} < V_{pinch}$ sempre)

                Modello: RC con $R_{DS,on}$
                $\quad \boxed{\Delta t = R_{DS,on} \cdot C_L \cdot \ln\!\left( \frac{V_{start}}{V_{end}} \right)}$

                con $R_{DS,on} = \frac{1}{2K(V_{GS}-V_T)}$

                \vspace{0.1cm}
                \textbf{\underline{CASO C: MISTA}} (attraversa $V_{pinch}$) $\leftarrow$ \textit{caso classico $t_p$}

                Modello: RC con $R_{eq}$ al pinch-off
                $\quad \boxed{t_p = 0.69 \cdot R_{eq} \cdot C_L}$

                con $R_{eq} = \frac{1}{K(V_{GS}-V_T)}$
