...
\begin{multicols*}{3}
    ...
    \begin{tikzpicture}
        ...
        %           \textbf{\color{red}$\bigstar$ REGOLA FONDAMENTALE - pMOS}

        %           \vspace{0.1cm}
        %           Affinché il pMOS sia \textbf{ON}, il \textbf{source deve essere più in alto del gate di almeno una soglia} ($|V_T|$):

        %           $$V_S - V_G \geq |V_T| \quad \Leftrightarrow \quad |V_{GS}| \geq |V_T|$$

        %           \vspace{0.05cm}
        %           {\footnotesize Il controllo $|V_{GS}| \geq |V_T|$ è \textbf{sufficiente} e include automaticamente la verifica della polarità corretta.}
        %       }}

                \vspace{0.2cm}
                \textbf{Step 1: Calcolare $|V_{GS}|$}
