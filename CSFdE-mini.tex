...
\usepackage{soul}
\definecolor{hlY}{RGB}{255,240,90}
\sethlcolor{hlY}
...
\SH{Metodo a ipotesi (linea dei valori)}
\F{\text{1. tutti OFF}\Rightarrow\text{range con }{-}|V_{BD}|{<}V_D{<}V_\gamma}{ogni diodo: 2 bordi (fwd e BD)}
\F{\text{2. segno di }V_D\text{: }{<}0\Rightarrow\text{solo bordo BD};\ {>}0\Rightarrow\text{solo fwd}}{l'altro lato \`e gi\`a vero: via}
\F{\text{3. al bordo cambia UN diodo: nuova mappa}}{bordi vecchi fuori $=$ fantasmi}
\hl{\textbf{4. CHECK: la verifica dell'ipotesi NUOVA deve ridare}}\par
\hl{\textbf{lo STESSO bordo a cui \`e morta la vecchia}} {\footnotesize\color{cue}(se no: clamp/stato sbagliato)}\par\vspace{3pt}
\F{\text{ON}\!\to\!{-}V_\gamma,\ \text{OFF}\!\to\!\text{apre},\ \text{BD}\!\to\!{-}|V_{BD}|}{fwd inchioda a $\pm0{,}7$, BD a $\pm V_{BD}$!}
\F{\text{incollaggio: }V_{out}\text{ continua al bordo}}{secondo check gratuito}
\end{fmbox}
...
% ================= CIRCUITO DELLE VARIAZIONI =================
\begin{fmbox}[cRC]{CIRCUITO DELLE VARIAZIONI (salti ai gradini)}
\SH{Costruzione (vale all'istante del gradino)}
\F{\text{gen.\ costanti (rail, DC): SPENTI}}{$\Delta$cost${=}0$: V$\to$massa, I$\to$aperto}
\F{C\to\text{CORTO}\ (\Delta V_C{=}0)}{il gradino resta l'unica sorgente}
\SH{Salto del nodo}
\F{\Delta V_{n}{=}\Delta I{\cdot}R_{vista}^{\Delta}\ \text{o}\ \Delta V_{in}{\cdot}\text{part.}^{\Delta}}{stessa R vista della $\tau$, con C${=}$filo}
\F{\text{C SERIE sul percorso: }\Delta\text{ passa}}{il nodo salta (AC coupling)}
\F{\text{C PARALLELO al nodo: }\Delta\!\to\!\text{massa}}{nodo inchiodato; tutta la $\Delta I$ in C}
\F{\text{a valle: }\Delta V{=}\Delta V_{n}\cdot\text{partitore}}{prima il nodo padrone, poi i figli}
\SH{Dopo il salto}
\F{V(t_0^+)=V(t_0^-)+\Delta V_{n}}{poi esponenziale (formula master)}
\F{\text{asintoto: DC nuova, C aperto}}{input com'era $+$ stessa topologia $\Rightarrow$ asintoto $=$ regime di $0^-$}
\F{\tau\text{ invariata se R viste immutate}}{topologia cambiata $\Rightarrow$ ricalcola}
\end{fmbox}
...
\F{\color{red}\textbf{INTERRUTTORE}\Rightarrow V_{DS}{\approx}0\Rightarrow\textbf{ohmica}}{\color{red}per progetto: non si verifica}
\F{\color{red}\textbf{DRIVER}\Rightarrow V_{DS}\textbf{ ampio}\Rightarrow\textbf{verifica zona}}{\color{red}spesso parte in saturazione}
\F{\color{red}\textbf{switch OK se } V_G\ge V_{S,max}{+}V_T{+}\vov}{\color{red}se no muore come il PTL}
...
\hl{\textbf{Sull'USCITA ($R_L$, $C_L$): MAI in $G_{id}$ n\'e in $G_{loop}'$}}\par
\hl{\textbf{($R_{out}{=}0$, $V_T$ ideale): solo $I_{out}$ / SR.}}\par
\hl{\textbf{Su $V^+$: invisibile in $G_{id}$ ($i_+{=}0$)\,---\,ma in $G_{loop}'$}}\par
\hl{\textbf{CONTA ($\beta^+$): niente corto virtuale con $V_T$!}}\par\vspace{3pt}
\F{G_{id}=-\tfrac{R_2}{R_1}}{invertente}
\F{G_{id}=1+\tfrac{R_2}{R_1}}{non invertente}
\F{G_{id}=1\ \text{(buffer)};\quad {-}\tfrac{R}{1+sRC}\ \text{(transimp.)}}{}
\F{{-}\tfrac{1}{sR_1C}\ \text{(integr.)};\quad {-}sR_2C\ \text{(deriv.)}}{polo / zero nell'origine}
...
\F{G_{reale}=\tfrac{G_{id}}{1+1/|G_{loop}|}}{ESATTA: la spezzata \`e approx!}
\F{\tfrac{\Delta G}{G_{id}}{=}\tfrac{1}{1+|G_{loop}(0)|};\ \varepsilon{=}\tfrac{V_{out,id}}{1+|G_{loop}(0)|}}{err.\ statico guad.\ finito ($\to$LSB)}
...
\hl{\textbf{Risoluzione $=$ su $\Delta$: offset/$V_B$/centro SPARISCONO}}\par
\hl{\textbf{(costanti): $\Delta V_{out}{=}G\cdot\Delta V_{in}$ e basta.}}\par
\hl{\textbf{Il centro conta solo per min/max/saturazione.}}\par\vspace{3pt}
...
\F{T_{hld}\le\tfrac{\tau_{hld}}{2^n}=\tfrac{R_{ADC}C_H}{2^n}}{droop $<1$ LSB (da FSR)}
\F{T_{hld}\ge T_{conv}{=}\tfrac{n}{f_{clk}}\Rightarrow f_{clk}\ge\tfrac{n}{T_{hld}}}{copre la conversione SAR}
\F{V_G>\vov^{min}+V_t+V_{S,max}}{gate sample (worst $V_S$)}
\F{C_H\ge\tfrac{T_{hld}(2^n-1)}{R_{ADC}}}{dimensionam.\ (sfrutta dinamica)}
\SY{$C_H$ hold \textbullet\ $R_{DS,on}$ switch \textbullet\ $n$ bit ADC}
\end{fmbox}

% ================= S&H: I TRE ERRORI =================
\begin{fmbox}[cAdc]{S\&H — i tre errori}
\SH{Fase, causa, formula}
{\footnotesize
\begin{tabular}{@{}p{1.0cm}p{2.5cm}l@{}}
sample & \raggedright settling: $C_H$ non arriva in tempo &
  $\varepsilon{=}\Delta V e^{-T_{smp}/\tau_{smp}}$\\[2pt]
S$\to$H & \raggedright charge inj.: carica del canale &
  $\Delta V{=}\Delta V_G\tfrac{C_{par}}{C_{par}+C_H}$\\[2pt]
hold & \raggedright droop: $C_H$ si scarica &
  $\Delta V{=}\tfrac{I_{leak}}{C_H}T_{hld}$\\
\end{tabular}\par}
\vspace{3pt}
\F{\text{ognuno}<\tfrac12\text{LSB},\ \textstyle\sum<1\,\text{LSB}}{spec tipica}
\F{C_H\!\uparrow\Rightarrow\text{inj},\text{droop}\!\downarrow\ \text{ma smp lento}}{TRADE-OFF centrale del S\&H}
\SH{Errori di guadagno}
...
\SY{$V_{FS}$ fondo scala \textbullet\ $C_{par}$ parassita \textbullet\ $I_{leak}$ perdite in hold}
\end{fmbox}
